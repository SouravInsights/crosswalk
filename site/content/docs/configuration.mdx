...
**`codegen.config.mjs`** is code. It defines structure: sources, outputs, safety
...
An array; today there is one output. `tools({ outDir })` writes the per-tool
files described in [Regeneration](/docs/regeneration). More outputs
(`form`, `reactHooks`) are on the roadmap.
