...
- **Writes and destructive tools start withheld.** The same working code is generated but
  commented out, and the tool is not registered: agents never see it, so it cannot dilute
  their choices or be picked by mistake. Enabling one is uncommenting the code and the
  registration fence, or flipping it on in the dashboard and regenerating.
  Set `safety.registerDisabled: true` if you want disabled tools to stay visible to agents
  as gated capabilities instead (calling one then returns a "this tool is disabled"
  message that points the agent at the human).
...
```bash
npx @webmcp-stack/codegen generate --dry-run   # see the findings without writing anything
```
