...
6. ~~User action, GitHub settings: rename repo `groundstate` → `webmcp-codegen`~~ **Superseded same day:** the monorepo will host multiple products, so it gets an umbrella brand: **Crosswalk**. The thesis in one line: agents poking the DOM is jaywalking; tools are the crosswalk. Codegen builds the crossing, audit keeps it safe, analytics counts the crossings. Verified free (Aug 31): github.com/crosswalk org, crosswalk.dev, @crosswalk npm scope. Plan: create the `crosswalk` org, register crosswalk.dev, transfer this repo and rename it `crosswalk` (posthog/posthog pattern; redirects preserve links), then make it public with the MIT license visible. Do this before submitting.
